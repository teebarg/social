              ...
              {users?.data.map((user) => (
                <TR key={user.id}>
                  <TD
                    className={cn(
                      "max-w-40 truncate",
                      !user.first_name ? "opacity-50" : "opacity-100",
                    )}
                  >
                    {user.first_name || "N/A"}
                    {currentUser?.id === user.id && (
                      <Chip title="You" className="ml-1" color="secondary" />
                    )}
                  </TD>
                  <TD
                    className={cn(
                      "max-w-40 truncate",
                      !user.last_name ? "opacity-50" : "opacity-100",
                    )}
                  >
                    {user.last_name || "N/A"}
                    {currentUser?.id === user.id && (
                      <Chip title="You" className="ml-1" />
                    )}
                  </TD>
                  ...
                  <TD>
                    <div className="flex gap-2">
                      <div
                        className={cn(
                          "w-1 h-1 rounded-50 self-center",
...
function Admin() {
  return (
    <div className="w-full px-2 py-4">
      <h2 className="text-xl text-center md:text-left pt-4">
        Users Management
      </h2>
