...
const AddTemplate = ({ isOpen, onClose }: AddTemplateProps) => {
    ...
    const mutation = useMutation({
        mutationFn: (data: NotificationTemplateCreate) => NotificationsService.createTemplate({ requestBody: data }),
        onSuccess: () => {
            ...
            {isOpen && (
                <Modal onClose={onClose}>
                    <div className="mx-auto w-full p-8">
                        <div>
                            <h2 className="text-lg font-semibold leading-6 text-default-800">Create Template</h2>
                            <form className="w-full mt-4" onSubmit={handleSubmit(onSubmit)}>
                                ...
                                <div className="flex justify-end gap-2 mt-8">
                                    <Button color="danger" className="min-w-32 " onClick={onClose} disabled={mutation.isPending}>
                                        Cancel
                                    </Button>
                                    <Button
                                        className="min-w-32"
