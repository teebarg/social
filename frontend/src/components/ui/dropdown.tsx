  ...
  useEffect(() => {
    if (state.isOpen && buttonRef.current && overlayRef.current) {
      const updatePosition = () => {
        const rect = (
          buttonRef.current as HTMLElement | null
        ...
        left = Math.max(
          4,
          Math.min(left, viewportWidth - contentRect.width - 4),
        )
  ...
  return (
    <React.Fragment>
      <button
        {...buttonProps}
        ref={buttonRef}
        className="inline-flex items-center text-default-500 cursor-pointer outline-none"
      >
        {trigger}
      </button>
      {state.isOpen && (
