...
const Tab: React.FC<TabProps> & {
    ...
    return (
        <TabContext.Provider value={{ activeTab, setActiveTab }}>
            <div className={cn("w-full max-w-2xl mx-auto p-6", className)}>{children}</div>
        </TabContext.Provider>
    );
};
...
const HeaderItem: React.FC<HeaderItemProps> = ({ index, children }) => {
    ...
    return (
        <button
            ...
            role="tab"
        >
            {activeTab === index && <span className="absolute bottom-0 inset-x-0 h-0.5 bg-blue-600" />}
            {children}
        </button>
    );
};
...
const Panel: React.FC<PanelProps> = ({ index, children }) => {
    ...
    return (
        <div
            className={`
                space-y-4 transition-opacity duration-200 ${activeTab === index ? "block" : "hidden"}
            `}
            role="tabpanel"
