...
const ActionsMenu = ({ type, value, disabled }: ActionsMenuProps) => {
    ...
    return (
        <>
            <Dropdown align="end" trigger={<EllipsisHorizontal />}>
                <div>
                    <div className="bg-default-100 rounded-lg shadow-md p-3 min-w-[100px] text-sm font-medium">
                        <div className="mb-2">
                            <button className="flex w-full items-center" onClick={editUserModal.open}>
                                <span className="mr-2">
                                    <PencilSquare />
                                </span>
                                <span>Edit</span>
                        ...
                        <div>
                            <button className={cn("flex w-full items-center text-rose-500")} onClick={deleteModal.open}>
                                <span className="mr-2">
                                    <Trash />
                                </span>
                                <span>Delete</span>
            ...
            {type === "User" ? (
                <EditUser user={value as UserPublic} isOpen={editUserModal.isOpen} onClose={editUserModal.close} />
            ) : (
                <EditItem item={value as ItemPublic} isOpen={editUserModal.isOpen} onClose={editUserModal.close} />
            )}
            {deleteModal.isOpen && (
                <Modal onClose={deleteModal.close}>
                    <Delete type={type} id={value.id} isOpen={deleteModal.isOpen} onClose={deleteModal.close} />
                </Modal>
            )}
            {/* <Menu>
