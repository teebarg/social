...
const Delete = ({ type, id, isOpen, onClose }: DeleteProps) => {
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            showToast.success("Success", `The ${type.toLowerCase()} was deleted successfully.`);
            onClose();
        },
        onError: () => {
            showToast.error("An error occurred.", `An error occurred while deleting the ${type.toLowerCase()}.`);
        },
        onSettled: () => {
                        ...
                        <div className="flex">
                            <div className="flex items-center">
                                <div className="flex grow flex-col gap-1">
                                    <h2 className="text-lg font-semibold leading-6 text-default-800">Delete {type}</h2>
                                </div>
                            </div>
                        </div>
                    </div>
