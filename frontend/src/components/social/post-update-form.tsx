...
const UpdatePost: React.FC<Props> = ({ post, onClose }) => {
    ...
    const [scheduledFor, setScheduledFor] = useState<Date | undefined>(new Date(post.scheduled_time as Date));
    const [isScheduling, setIsScheduling] = useState<boolean>(!!post.scheduled_time);

    const mutation = useMutation({
        mutationFn: (data: DraftUpdate) => DraftService.update({ id: post.id, requestBody: data }),
        onSuccess: () => {
        ...
        <div className="bg-content1 rounded-xl shadow-sm py-6 px-2">
            <div className="space-y-4">
                <div>
                    ...
                    <Input value={title} onChange={(e: any) => setTitle(e.target.value)} id="title" name="title" className="max-w-sm" required />
                </div>
                <textarea
                    value={content}
